...
时间：2026-07-02 15:41:35 - 15:43:11。

检测前提：

- Windows 用户代理：`127.0.0.1:7897`。
- 当前 PowerShell 环境变量中未设置 `HTTP_PROXY` / `HTTPS_PROXY`。
- WinHTTP 为直连。
- 检测只访问公网 IP 查询服务，不调用微信接口，不创建草稿。

检测方法：

- 连续 12 次采样，每次间隔 5 秒。
- 同时检查直连出口与显式代理出口。
- 显式代理使用 `curl.exe -4 -x http://127.0.0.1:7897`。
- 代理出口分别用 `api.ipify.org`、`ipv4.icanhazip.com`、`checkip.amazonaws.com` 三个服务交叉验证。

结果：

- 12 次直连出口均为 `64.118.158.227`。
- 12 次代理出口均为 `64.118.158.227`。
- 三个公网 IP 查询服务返回一致。
- 设置临时环境变量 `HTTP_PROXY=http://127.0.0.1:7897`、`HTTPS_PROXY=http://127.0.0.1:7897` 后，公网出口仍为 `64.118.158.227`。
- `md2wechat config validate` 在上述代理环境下通过。
