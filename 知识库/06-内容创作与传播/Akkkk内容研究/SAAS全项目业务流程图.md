---
title: SAAS 全项目业务流程图
aliases:
  - SAAS 业务流程图 (code-review-graph official API)
created: 2026-05-24
updated: 2026-06-27
source_id: src-20260524-code-review-graph-saas-flow
sources:
  - raw/sources/src-20260524-code-review-graph-saas-flow/original.md
  - 知识库/sources/src-20260524-code-review-graph-saas-flow.md
status: migrated-to-synthesis
source_level: none
---
# SAAS 全项目业务流程图

> 兼容旧入口。原页面曾包含 code-review-graph 生成的完整图谱输出，现已归档到 `raw/sources/src-20260524-code-review-graph-saas-flow/original.md`。

## 当前入口

- 主来源页：[[知识库/sources/src-20260524-code-review-graph-saas-flow|SAAS code-review-graph 业务流程图来源]]
- 综合页：[[知识库/syntheses/SAAS代码图谱业务流程图|SAAS代码图谱业务流程图]]
- 原文证据：`raw/sources/src-20260524-code-review-graph-saas-flow/original.md`

## 迁移说明

- 原文层不改写、不摘要替换，已复制到 raw 并校验哈希一致。
- 本页保留为旧链接兼容入口，不再承载 7000+ 行工具输出。
- 后续更新图谱时，应生成新的原文证据并更新来源页，不在本页粘贴完整 JSON。
