...
- 继续执行: 用户明确要求执行后，已生成 `tmp/wechat-sop-smoke/article.html`，本地检查未命中 `<style|<head|<link|@import`；执行 `md2wechat test-draft ... --json` 失败，返回 `TEST_DRAFT_COVER_FAILED / errcode=40164 invalid ip 42.84.232.254 not in whitelist`，当前阻塞点为微信公众号后台 IP 白名单
- IP 补证: 历史 AI 模式流程文档记录 2026-05-03 配置白名单 `42.84.233.154` 后 access_token 与 test-draft v1/v2 成功；本次失败出口为 `42.84.232.254`；当前普通公网探测曾返回 `64.118.158.227`，但未经过微信接口验证，且本机存在 `127.0.0.1:7897` 用户代理，后续应迁移到固定公网 IP 的云服务器/NAT/EIP，或显式固定 md2wechat 代理出口后再验证
- 代理检测: 2026-07-02 15:41:35-15:43:11 连续 12 次采样，直连与显式代理 `http://127.0.0.1:7897` 出口均为 `64.118.158.227`，`api.ipify.org`、`ipv4.icanhazip.com`、`checkip.amazonaws.com` 三个服务一致；临时设置 `HTTP_PROXY`/`HTTPS_PROXY` 后出口仍为 `64.118.158.227`，`md2wechat config validate` 通过。建议本轮公众号 IP 白名单加入 `64.118.158.227`，加白后执行 `test-draft` 前仍需预检出口 IP
- 小红书可行性补证: 当前 `md2wechat-skill` 的“小绿书/newspic”是微信公众号图片消息草稿，源码最终调用微信 `draft/add`，未发现 `xiaohongshu`/`xhs`/`rednote` 平台适配器；小红书开放平台公开文档以商家/服务商/电商工具路径为主，服务市场与内容工具类目存在企业服务商准入要求。阶段性结论为：当前项目不能直接实现小红书平台自动发布，只能复用内容生成与素材整理能力；合规实现需先取得小红书官方开放平台相关权限并新增独立 XHS adapter

## [2026-07-02] maintenance | 认知文章 raw 原文补档

- 改动范围: `raw/sources/人智认知系列/`、`知识库/05-认知与成长/01-人智认知系列/*.md`、`知识库/_review/unresolved-items.md`、`知识库/01-总览与索引/知识库目录结构（2026版）.md`、`index.md`、`log.md`
- 改动原因: 用户提供 `C:\Users\caojianing\Downloads\界面认知文章整理.zip`，要求将认知相关所有文章整理到 raw，补齐缺失项，已有文件不重复放入
- 处理结果: 压缩包共 135 个文件；`中美博弈系列` 83 个与 `醒与悟系列` 8 个均已存在且 SHA256 相同，未覆盖；新增 `人智认知系列` 44 个原始文件，保留压缩包原始文件名
- 来源链路: 已将 36 个 `知识库/05-认知与成长/01-人智认知系列/` 正式页的 `## 原文链接` 从“缺失待核查”改为真实存在的 `raw/sources/人智认知系列/...` Wiki 链接；婚姻/教育删减、潜意识、抱猴心理等文件名按压缩包真实文件名对齐
- 跳转修正: 复核发现 Obsidian 点击部分 `.docx` Wiki 链接会误创建 0 字节 `.md` 空笔记；受 raw append-only 门禁约束，`16-案例分析.docx.md`、`33-人智—抱猴心理.docx.md`、`33-人智—抱猴心理.docx 1.md` 已保留但不参与正式页链接；已为 36 个正式页补充 `打开原文` Markdown 附件链接，同时保留 Wiki 反链
- 待复查项: 本次压缩包仍未提供 `中美博弈系列` 的 #046 与 #081-087 原始文件；`人智认知系列` 的 `20-40.docx/pdf`、`21-40合集.doc`、`（1-20篇合集）.doc` 等合集文件已归档，但未单独提升为正式知识页

## [2026-07-02] maintenance | GitHub 热门旧报告迁移清理
