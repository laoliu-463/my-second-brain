...
- 来源页需保证可追踪：至少有 `source_id/raw_path/canonical_url/raw_sha256` 等可校验字段（可在对应来源页中实现）
- `index.md` 的分类名称必须与实际分类口径一致，不得出现目录不存在的分类占位

### 建议

- 页面命名优先中文语义名，必要时保留编号前缀
- 来源与证据链优先在 `知识库/90-来源与映射/` 及 `wiki/sources`（或等价目录）中可回溯
...
- `知识库/90-来源与映射/` 与来源页要形成可恢复追踪
- 一个来源页对应一个或多个知识页时，优先给出稳定唯一 source_id
- raw 文件缺失应在报告中落账，先标记待核查，不可伪造链接
